...
const axios = require('axios'); // Tambahkan axios untuk request webhook
const app = express();
const port = 8080;

let publicUrl = '';
const DOWNLOAD_DIR = path.join(__dirname, 'downloads');
//const WEBHOOK_URL = 'https://webhook-test.com/23258ea30409100e4c8751d3fa3b47ca';
const WEBHOOK_URL = 'https://apprehensive-doctor-97.webhook.cool';

if (!fs.existsSync(DOWNLOAD_DIR)) {
    fs.mkdirSync(DOWNLOAD_DIR, { recursive: true });
}

// Fungsi untuk mengirim notifikasi ke webhook
async function sendWebhookNotification(title, status, errorDetails = '') {
    const payload = {
        embeds: [{
            title: 'Notifikasi Download',
            description: `**Status:** ${status}\n**Judul:** ${title}`,
            color: status.includes('Berhasil') ? 65280 : 16711680,
            fields: []
        }]
    };
    ...
    try {
        ...
        console.log('✅ Notifikasi webhook berhasil dikirim.');
    } catch (error) {
        console.error('❌ Gagal mengirim notifikasi webhook:', error.message);
    }
}
...
app.post('/download', (req, res) => {
    ...
    exec(ytDlpCommand, { maxBuffer: 1024 * 1024 * 100 }, (error, stdout, stderr) => {
        let titleMatch = stdout.match(/\[download\] Destination: (.+)/) || stderr.match(/\[download\] Destination: (.+)/);
        const videoTitle = titleMatch ? path.basename(titleMatch[1]).replace(/\..+$/, '') : 'Video tidak diketahui';

        if (error) {
            console.error(`Error yt-dlp: ${error.message}`);
            // Kirim notifikasi webhook jika gagal
            sendWebhookNotification(videoTitle, 'Gagal', stderr.substring(0, 500));
        ...
        if (downloadedFiles.length === 0) {
            // Kirim notifikasi webhook jika tidak ada file yang dihasilkan
            sendWebhookNotification(videoTitle, 'Gagal', 'Tidak ada file yang dihasilkan.');
            return res.status(404).send(`<p class="error">Gagal mendownload: Tidak ada file yang dihasilkan.</p><a href="/">Coba Lagi</a>`);
        }
        
        // Kirim notifikasi webhook jika berhasil
        sendWebhookNotification(videoTitle, 'Berhasil');

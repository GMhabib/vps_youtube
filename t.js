// website.js (Modifikasi untuk Vercel)

const express = require('express');
const ytdl = require('ytdl-core'); // Library Node.js murni untuk YouTube
const app = express();
...
// 2. require('fs')
// 3. require('path') untuk operasi file
// 4. app.listen() dan fungsi main() dengan Serveo
// ----------------------------
...
app.get('/', (req, res) => {
    // Tampilan HTML (tidak diubah, karena ini murni HTML)
    res.send(`
<!DOCTYPE html>
<html lang="id">
<head>
...
app.post('/download', async (req, res) => {
    ...
    try {
        ...
        if (format === 'mp4') {
            ...
            // Pilihan untuk kualitas video dan audio terbaik yang digabung
            ytdlOptions = { quality: 'highestvideo' }; 
        } else {
            filename = `${title}.mp3`;
            contentType = 'audio/mpeg';
            // Filter hanya untuk audio, kualitas tertinggi
            ytdlOptions = { filter: 'audioonly', quality: 'highestaudio' };
        }

        // 2. Atur Header Respon untuk Download
        res.header('Content-Disposition', `attachment; filename="${filename}"`);
        res.header('Content-Type', contentType);
        // Header ini penting untuk mengizinkan streaming besar
        res.header('Transfer-Encoding', 'chunked'); 

        // 3. Stream Data Video/Audio Langsung ke Klien
        const downloadStream = ytdl(youtubeUrl, ytdlOptions);

        downloadStream.on('error', (err) => {
            console.error('[YTDL Error]:', err.message);
            // Coba kirimkan status error jika belum ada data yang terkirim
            if (!res.headersSent) {
                res.status(500).send(`Gagal memproses video: ${err.message}`);
            } else {
                // Jika sudah ada data yang terkirim, kita hanya perlu mengakhiri respon
                res.end();
            }
        });
